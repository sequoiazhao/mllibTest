...
import com.AllTest.mllibLDA.PreUtils
...
object VectorTFIDFPipeline {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data2 = sc.textFile(inPath, minPart).zipWithIndex().map(_.swap)
    val baseExpr = """[^\u4e00-\u9fa5]""".r
    val data3 = data2.map { x =>
      val xt = baseExpr.replaceAllIn(x._2, "")
      val list = StandardTokenizer.segment(xt)
      CoreStopWordDictionary.apply(list)
      val temp = list.map(x => x.word.replaceAll(" ", "")).mkString(",").split(",")
      (x._1, temp.toSeq)
    }.filter(_._2.nonEmpty)


   data3.take(10).foreach(println)
    ...
    val strarr = Array("取得", "工业", "上海", "大战", "服务", "比赛", "情形", "工作", "生活", "赛事")

//    strarr.foreach(x=>
//      modelVec.findSynonyms(x,10).foreach(y=>println(x,y))
//    )

    val vecs = modelVec.getVectors

    vecs.show(false)

    //val s1 = vecs.where("word='取得'").select("vector").printSchema()
