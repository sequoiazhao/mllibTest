...
/**
  * @author zhaoming on 2018-04-12 15:37
  **/
object TestMLlibLocalEnglish {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mdd = sc.textFile("D:\\dataframeData\\englocal01_702")
    ...
    val tdd = sc.textFile("D:\\dataframeData\\englocal02_702")
          ...
          """
                  ...
                  },
                  {
                    "sltr": {
                      ...
                      "featureset": "2_1_2_v40_vod_english_similarity",
                      "params": {
     ...
     "objname"]
      ...
      val res = logArray.zipWithIndex.map { x =>
        ...
        if (strx =="""[{"name":"bm25"},{"name":"startoffset"},{"name":"ratio"}]"""
          || strx =="""[{"name":"bm25"},{"name":"startoffset","value":0.0},{"name":"ratio"}]"""
          || strx =="""[{"name":"bm25"},{"name":"startoffset","value":0.0},{"name":"ratio"}]"""
          || strx =="""[{"name":"bm25"},{"name":"startoffset","value":0.0},{"name":"ratio","value":0.0}]""") {
          println(strx)
          Array(0.0, 0.0, 0.0)
    ...
    //    val sqlContext = SQLContext.getOrCreate(MyContext.sc)
    //    import sqlContext.implicits._
    val datax = trainingData.toDF("label", "features")
    ...
    val indexdata = featureIndexer.transform(datax)
    indexdata.show(200, false)
