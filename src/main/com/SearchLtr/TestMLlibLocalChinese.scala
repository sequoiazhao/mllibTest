...
object TestMLlibLocalChinese {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mdd = sc.textFile("D:\\dataframeData\\chnlocal01_702")
    ...
    SampleDataOriginal.show(1000,true)

    val tdd = sc.textFile("D:\\dataframeData\\chnlocal02_702")
    ...
    val data = relData.rdd.take(relData.count.toInt).map { mex =>
          ...
          """
                  ...
                  },
                  {
                    "sltr": {
                      ...
                      "featureset": "2_1_2_v40_vod_chinese_similarity",
                      "params": {
      ...
      val result = Http("http://10.18.216.249:9214/vod_online/_search?size=100")
...
//        println(x._1)
//        print(idArray(x._2))
//        val strx = x._1.toString.trim
//        if (strx =="""[{"name":"bm25"},{"name":"startoffset"}]""" || strx =="""[{"name":"bm25"},{"name":"startoffset","value":-1.0}]""") {
//          Array(0.0, 100.0)
//        } else {
//          val strs = strx.replace("""[{"name":"bm25","value":""", "")
//            .replace(""""name":"startoffset","value":""", "").replace("""}]""", "")
//            .replace("""{""", "").replace("""}""", "")
//          println(strs)
//          strs.split(",").map(_.toDouble)
//
//        }
//
//      }
      ...
      val res = logArray.zipWithIndex.map { x =>
        ...
        val strx = x._1.toString.trim
        if (strx =="""[{"name":"bm25"},{"name":"startoffset"},{"name":"ratio"}]"""
          || strx =="""[{"name":"bm25"},{"name":"startoffset","value":-1.0},{"name":"ratio"}]"""
          || strx =="""[{"name":"bm25"},{"name":"startoffset","value":100.0},{"name":"ratio"}]"""
          || strx =="""[{"name":"bm25"},{"name":"startoffset","value":100.0},{"name":"ratio","value":0.0}]""") {
          println(strx)
          Array(0.0, 0.0, 0.0)

        } else {
          val strs = strx.replace("""[{"name":"bm25","value":""", "")
            .replace(""""name":"startoffset","value":""", "")
            .replace(""""name":"ratio","value":""", "")
            .replace("""}]""", "")
            .replace("""{""", "").replace("""}""", "")
          println(strs)
          strs.split(",").map(_.toDouble)
      ...
      val LogValueArray = res

      idArray.zipWithIndex.map { x =>
        (mex.get(0).toString, x._1.toString
          , LogValueArray(x._2)(0).toString.toDouble
          , LogValueArray(x._2)(1).toString.toDouble
         , LogValueArray(x._2)(2).toString.toDouble)
      }
    ...
    val sdata = MyContext.createDataFrame(data.toSeq)
      .toDF("srcsearchkey", "mediaid", "tfidf","startoffset", "ratio")
    ...
    boostingStrategy.treeStrategy.setCheckpointInterval(10)
