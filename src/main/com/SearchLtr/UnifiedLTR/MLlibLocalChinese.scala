...
object MLlibLocalChinese {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mdd = sc.textFile("D:\\dataframeData\\chnlocal01_722")
    ...
    val tdd = sc.textFile("D:\\dataframeData\\chnlocal02_722")

    val relData = tdd.map { x =>
      val s1 = x.replace("[", "").replace("]", "").replace(", ", ":").split(",")
      (s1(0), s1(1),s1(2))
    }.toDF("srcsearchkey", "mediaid","searchlable")
    relData.show(false)
    ...
    val data = relData.rdd.take(relData.count.toInt).map { mex =>
      ...
      val searchx =
        """{
         ...
         """
              "fields":[""" + mex.get(2).toString.replace("#", ",") +
      """]
                      }
                    }
                  }
      ...
      idArray.zipWithIndex.map { x =>
        ...
        ,LogValueArray(x._2)(3).toString.toDouble)
      }
    ...
    val sdata = MyContext.createDataFrame(data.toSeq)
      .toDF("srcsearchkey", "mediaid", "tfidf","startoffset", "ratio","m_bm25")
